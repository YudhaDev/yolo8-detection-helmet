...
class ScanRFID:
    ...
    def scan2(self, debug=False):
        ...
        mode = "scan masuk"
        # Cek dulu sudah scan atau belum
        base_url = "http://127.0.0.1:5000/"
        combined_url = base_url + "check-sudah-scan/" + rfid_number_scan_masuk[0] + '/' + mode
        response = requests.get(combined_url)

        print(f"respon json: {response.json()}")
        ...
        if response.json():
            ...
            # Cek kehadiran
            base_url = "http://127.0.0.1:5000/"
            response = requests.get(base_url + "cek-kehadiran/"+rfid_number_scan_masuk[0])
            print(response.text)
    ...
    def scanRegister2(self):
        ...
        while True:
            if len(state_store.global_rfid_number)>5:
                # check dulu apa rfidnya sudah ada atau tidak
                base_url = "http://127.0.0.1:5000/"
                combined_url = base_url + "cekTerdaftar/"+state_store.global_rfid_number
                # print(combined_url)
                response = requests.get(combined_url)
                if response.status_code == 404:  # brrti blom terdaftar
                    state_store.global_socketio_object.emit('rfid-register', state_store.global_rfid_number)
                    break
                else: # sudah terdaftar
                    state_store.global_socketio_object.emit('rfid-register-terdaftar', state_store.global_rfid_number)
                    print("")
    ...
    def scanPulang2(self):
        ...
        while True:
            if len(state_store.global_rfid_number)>5:
                # Cek dulu sudah scan atau belum
                base_url = "http://127.0.0.1:5000/"
                combined_url = base_url + "check-sudah-scan/"+state_store.global_rfid_number+'/'+mode
                response = requests.get(combined_url)
